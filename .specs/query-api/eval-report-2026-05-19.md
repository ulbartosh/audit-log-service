...
|---|---|---|
| Required sections present | PASS | `requirements.md` has a Problem paragraph, persona-specific acceptance criteria, and an explicit Out of scope section. |
| ACs follow EAR style | PASS | `requirements.md` writes each AC as one `When`, `If`, or `The system shall` EAR sentence. |
| ACs are atomic and observably testable | PASS | `requirements.md` ACs each describe one externally visible filter, validation, response-shape, ordering, or pagination behavior. |
...
| Every task has `Refs` to ACs and design sections | PASS | `tasks.md` gives each numbered task design refs and specific AC refs or enabled AC refs, with a coverage table tying the tags back to requirements. |
