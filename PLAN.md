...
Reviewer: `gemini-code-assist`. Four threads, addressed in commit `357ecc3` and resolved on GitHub.

| # | File | Comment summary | Fix | Thread |
| --- | --- | --- | --- | --- |
| 1 | `service/RetentionService.java` | `RETURNING id` is unnecessary once the service uses `jdbcTemplate.update()`. | Switched the archival SQL execution from `query(... RowMapper<UUID>)` to `update(...)`, removed `RETURNING id`, and used the affected-row count to drive batching. | resolved |
| 2 | `service/RetentionService.java` | `EntityManager.clear()` is a blunt side effect and the dependency can be removed. | Removed the `EntityManager` field and constructor dependency; the copy-only archival path no longer clears the persistence context. | resolved |
| 3 | `service/RetentionService.java` | `update()` is more efficient than `query()` for this batch insert. | Same refactor as above: `jdbcTemplate.update()` now handles the archival SQL directly and the loop uses the update count for continuation/termination. | resolved |
| 4 | `service/RetentionServiceTest.java` | Unit tests should mock `jdbcTemplate.update()` after the service refactor. | Updated `RetentionServiceTest` to stub `update(...)` counts instead of `query(...)`, and simplified the expectations accordingly. | resolved |
